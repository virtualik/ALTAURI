package Src.Prog.Com.Atoms.Core {
    ...
    /**
     * Universal view class - теперь только для Contact системы.
     */
    public class AtomView extends Sprite {
        ...
        // 🔥 УДАЛЕНО: PIN_VERTICAL_OFFSET, CONTACT_VERTICAL_OFFSET и _useContactSystem
        // Теперь используем только Contact систему
        ...
        /**
         * 🔥 ИСПРАВЛЕНО: Обновляет ContactView для всех контактов атома.
         * Теперь используем только Contact систему.
         */
        private function updateContacts(config:Object):void {
            ...
            for (var j:int = 0; j < _atom.contactInputs.length; j++) {
                ...
                // Располагаем равномерно по левой стороне
            ...
            for (var k:int = 0; k < _atom.contactOutputs.length; k++) {
                ...
                // Располагаем равномерно по правой стороне
        ...
        private function onMouseDown(event:MouseEvent):void {
            // Проверяем, кликнули ли на ContactView
            var target:DisplayObject = event.target as DisplayObject;
            while (target && target != this) {
                if (target is ContactView) {
                    // Клик на контакте - пусть он сам обрабатывает
                    return;
                }
        ...
        /**
         * 🔥 УДАЛЕНО: Метод findPinByName больше не нужен
         */
        ...
        private function onDrag(event:MouseEvent):void {
            ...
            Impulsys.emit(new Impulse("ATOM_MOVED", {
                oldAtom: _atom,
                newAtom: newAtom,
                updateLinks: true, // 🔥 ИСПРАВЛЕНО: updateTracks → updateLinks
                isDragging: true
            }));
            ...
            // 🔥 ИСПРАВЛЕНО: Обновляем только контакты
            updateVisualsDuringDrag();
        }

        /**
         * 🔥 ИСПРАВЛЕНО: Обновляем только ContactView
         */
        private function updateVisualsDuringDrag():void {
            var config:Object = getVisualConfig();
            updateContacts(config); // Всегда обновляем контакты
        }
        ...
        public function updateVisuals():void {
            ...
            updateContacts(visualConfig); // 🔥 ВСЕГДА обновляем контакты
        }
