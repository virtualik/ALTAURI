...
/**
* MAIN v3.4 (Points-of-Interest Autosave + Full-Stack Save + Crash Traps)
* Application entry point and main coordinator.
*
* ═══════════════════════════════════════════════════════════════════════════
* ═══════════════════════════════════════════════════════════════════════════
* ═══════════════════════════════════════════════════════════════════════════
* v3.2 CHANGES (Registry Alias Kill — Test 1 crash root, 2026-08-24)
* ═══════════════════════════════════════════════════════════════════════════
*
*  saveNewNamedAssembly() mutated the live blueprint id ("asm_53597" ->
*  "Direct") and registered the NEW key without removing the old one. One
*  live Blueprint under two registry keys produced: duplicate Assemblies
*  menu entries, a bypassed recursion guard (string compare) and the
*  self-nesting stack-overflow crash. The stale key is now removed
*  (identity-checked) whenever the id changes.
*
* ═══════════════════════════════════════════════════════════════════════════
* v3.3 CHANGES (Full-Stack Exit Save — lost-root fix, 2026-08-25)
* ═══════════════════════════════════════════════════════════════════════════
*
*  PROBLEM: [X]-exit (and any saveOnExit() path) saved ONLY the
*  currently open editor level. Exiting from inside a nested assembly
*  saved that assembly to the Library but silently DROPPED the root
*  schematic — on the next launch the user found an empty root and had
*  to rebuild the whole harness (field evidence 2026-08-25, part 1).
*
*  FIX: saveOnExit() now walks the ENTIRE editor stack top-to-bottom:
*  every nested assembly gets the full sync funnel (refresh names,
*  displayNames, live values, template IDs, registry) and is saved to
*  the Library; the root is saved through the existing selfrun path.
*
* ══════════════════════════════════════════════════════════════════════════════
* v3.4 CHANGES (Points-of-Interest Autosave + Root-Save Fix, 2026-08-25)
* ══════════════════════════════════════════════════════════════════════════════
*
*  (1) ROOT-SAVE FIX (v3.3 regression, caught in review): the v3.3 walk
*      synced/saved every NESTED level, but the tail still called
*      saveCurrentContext(), which saves the ROOT only when the user
*      stands ON it (stackLength == 1). [X] from depth >= 2 STILL
*      dropped the root. saveOnExit() is now a thin wrapper over
*      saveEntireStack(), which ends with an unconditional root
*      funnel (sync kit + saveSelfrun).
*
*  (2) POINTS-OF-INTEREST AUTOSAVE (user spec): add/remove Atom and
*      add/remove Assembly trigger a debounced (500 ms) FULL-STACK
*      save. Wired via the Impulsys bus: ATOM_RESTORED / ATOM_DELETED
*      cover create, delete, paste, group, ungroup, undo and redo
*      — one listener, all entry points. Quarantine teardown
*      (payload.reason != null) is skipped. Device-panel widget
*      add/remove NOT covered yet (needs DevicePanel.hx).
*
...
// v3.4: Points-of-Interest autosave — structural changes
// (atom/assembly add-remove) trigger a debounced full-stack save
// (see onStructuralAutosave below).
        private var _autosaveTimer:haxe.Timer = null;
...
// =========================================================================
        ...
        */
        private function saveCurrentContext():Void
        {
                ...
                if (isRoot)
                {
                        saveRootProject(_editorContext.currentAssembly, _editorContext.currentEditor);
                }
                else {
...
// ═══ v3.4: ROOT SAVE funnel, extracted from saveCurrentContext().
// Full-stack paths (exit, structural autosave) must save the ROOT even
// while the user stands INSIDE a nested assembly — v3.3 regressed
// here (see saveEntireStack).
        private function saveRootProject(rootAsm:Assembly, rootEditor:Dynamic):Void
        {
                log("Saving Root...");
...
// v3.0: Use DisplayConfig instead of _isPanelMode
                ...
                var viewState = rootEditor != null ? rootEditor.getViewState() : {x: 0.0, y: 0.0, zoom: 1.0};
        ...
        */
        ...
        /**
        * v3.4: FULL-STACK SAVE — shared by exit ([X]) and the structural
        * autosave. Walks the editor stack top-to-bottom, syncs + saves every
        * NESTED assembly to the Library, then unconditionally funnels the ROOT
        * through saveRootProject() (v3.3 regression fix: the root was saved
        * only when the user stood on it, stackLength == 1).
        */
        private function saveEntireStack():Void
        {
// ═══ v3.3: FULL-STACK SAVE. Previously only the CURRENT level was
// saved — exiting from inside a nested assembly ([X]) left the ROOT
// unsaved and the whole root session was lost on relaunch. Walk the
// editor stack top-to-bottom: sync + save every NESTED assembly to the
// Library, then save the root through the existing path below.
                var entries = _editorContext.getStackEntries();
                var level:Int = entries.length - 1;
                while (level >= 1)
                {
                        var asm = entries[level].assembly;
                        if (asm != null && asm.blueprint != null
                                && asm.blueprint.id != "selfrun" && asm.blueprint.id != "loaded_asm")
                        {
                                try
                                {
                                        ...
                                        _projectManager.saveAssemblyToLibrary(asm);
                                }
                                catch (e:Dynamic)
                                {
                                        log('saveEntireStack: failed to save nested assembly "${asm.blueprint.id}": $e');
                                }
                        }
                        level--;
                }

// ═══ v3.4 FIX: THE ROOT ITSELF. The v3.3 walk stops at level 1 and
// the old tail called prepareCurrentAssemblyForSave()/saveCurrentContext()
// on the CURRENT level — which saves the ROOT only when
// stackLength == 1. Exiting [X] from depth >= 2 saved every nested
// assembly and silently DROPPED the root. Funnel the root through the
// same sync kit and write Selfrun unconditionally, whatever level the
// user stands on.
                if (entries.length > 0)
                {
                        var rootAsm = entries[0].assembly;
                        if (rootAsm != null && rootAsm.blueprint != null)
                        {
                                try
                                {
                                        rootAsm.refreshExternalPortNames();
                                        rootAsm.syncDisplayNamesToBlueprint();
                                        if (rootAsm.displayName != null
                                                        && rootAsm.displayName != rootAsm.blueprint.name)
                                        {
                                                rootAsm.blueprint.name = rootAsm.displayName;
                                        }
                                        rootAsm.syncAtomValuesToBlueprint();
                                        rootAsm.syncConnectionsToTemplateIds();
                                        library.AtomRegistry.registerBlueprint(rootAsm.blueprint.id, rootAsm.blueprint);
                                        saveRootProject(rootAsm, entries[0].editor);
                                        utils.Trap.log("EXIT", "root funnel done");
                                }
                                catch (e:Dynamic)
                                {
                                        log('saveEntireStack: failed to save ROOT: $e');
                                }
                        }
                }
                utils.Trap.log("EXIT", "saveEntireStack done");
        }

// =========================================================================
// v3.4: POINTS-OF-INTEREST AUTOSAVE
// =========================================================================
        /**
        * Structural change on the bus: an atom or assembly instance was
        * added to / removed from a scheme. ATOM_RESTORED / ATOM_DELETED
        * cover create, delete, paste, group, ungroup, undo AND redo —
        * every structural mutation flows through these two impulses, so
        * ONE listener covers all entry points. Debounce 500 ms (a burst of
        * N events coalesces into ONE save), then a FULL-STACK save (every
        * nested assembly + the root).
        */
        private function onStructuralAutosave(impulse:Impulse):Void
        {
                // Startup guard — do not react to anything fired before the
                // UI is fully constructed.
                if (!_uiReady) return;

                // Quarantine teardown carries "reason" — emergency path, do
                // not persist possibly-corrupt state through the autosave funnel.
                if (impulse != null && impulse.data != null && impulse.data.reason != null) return;
                ...
                _autosaveTimer = haxe.Timer.delay(() -> {
                                _autosaveTimer = null;
                                performStructuralAutosave();
                }, 500);
        }

        private function performStructuralAutosave():Void
        {
                utils.Trap.log("AS-SAVE", "structural autosave begin stack=" + _editorContext.getStackLength());
                try
                {
                        saveEntireStack();
                        utils.Trap.log("AS-SAVE", "structural autosave done");
                }
                catch (e:Dynamic)
                {
                        utils.Trap.log("AS-SAVE", "structural autosave FAILED: " + e);
                }
        }
...
// every existing blueprint.name (native + custom).
                ...
                var bp = _editorContext.currentAssembly.blueprint;
                var oldBpId:String = bp.id;
                bp.id = uniqueSafeId;
                bp.name = uniqueName;

// ═══ v3.2 FIX A (Registry Alias Kill): the id mutation above orphans the OLD
// registry key (e.g. "asm_53597") while the SAME live Blueprint gets
// registered under the NEW key. The stale alias caused duplicate menu
// entries and the self-nesting crash (Test 1, 2026-08-24). Remove the old
// key when it still points at THIS blueprint object (identity check — never
// touch an unrelated blueprint that merely shares the old id).
                if (oldBpId != null && oldBpId != uniqueSafeId
                        && library.AtomRegistry.exists(oldBpId)
                        && library.AtomRegistry.get(oldBpId) == bp)
                {
                        library.AtomRegistry.remove(oldBpId);
                        log('Registry: removed stale alias key "$oldBpId" (blueprint re-keyed as "$uniqueSafeId")');
                }
...
// v3.4: Points-of-Interest autosave — structural bus events
                Impulsys.subscribeToImpulse(EventType.ATOM_RESTORED, onStructuralAutosave);
                Impulsys.subscribeToImpulse(EventType.ATOM_DELETED, onStructuralAutosave);
...
// v3.0: Reset DisplayConfig mode
                ...
                Impulsys.subscribeToImpulse(EventType.VALUE_COMMITTED, onValueCommitted);
                Impulsys.subscribeToImpulse(EventType.DEVICE_WINDOW_CHANGED, onDeviceWindowChanged);
                // v3.4: autosave subscriptions must survive Impulsys.clear()
                Impulsys.subscribeToImpulse(EventType.ATOM_RESTORED, onStructuralAutosave);
                Impulsys.subscribeToImpulse(EventType.ATOM_DELETED, onStructuralAutosave);
