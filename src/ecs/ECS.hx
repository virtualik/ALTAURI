 ...
 * │   ┌─────────────────────────────────────────────────────────────────┐   │
 * │   │  API:                                                           │   │
 * │   │  - init()                    → Initialize ECS render layer      │   │
 * │   │  - register(id, sprite, x,y) → Register a renderable node       │   │
 * │   │  - unregister(id)            → Unregister a node (on delete)    │   │
 * │   │  - updatePosition(id, x, y)  → Update node position             │   │
 * │   │  - setSelected(id, selected) → Set selection state              │   │
 * │   │  - isSelected(id) → Bool     → Check if selected                │   │
 * │   │  - render()                  → Render all nodes (every frame)   │   │
 * │   │  - getSelected() → Array     → Get all selected atom IDs        │   │
 * │   │  - getInRect(x,y,w,h)        → Spatial query for lasso          │   │
 * │   │  - clearSelections()         → Clear all selections             │   │
 * │   │  - setEnabled(enabled)       → Enable/disable rendering         │   │
 * │   │  - reset()                   → Full reset                       │   │
 * │   │  - getWorld() → World        → Direct World access              │   │
 * │   └─────────────────────────────────────────────────────────────────┘   │
 * │                                                                         │
 * │   Usage:                                                                │
 * │   ───────                                                               │
