...
*
* ┌─────────────────────────────────────────────────────────────────────────┐
* * │  v2.0.1 HOTFIX (Task 103 - an MSVC compilation fix):                     │
* │                                                                         │
* * │  winhttp.h does not include windows.h itself: its line 54                │
* * │  "typedef LPVOID HINTERNET;" requires basic Win32 types                  │
* * │  (LPVOID / DWORD / APIENTRY) that were not yet in the TU -               │
* * │  hence the C4430 / C2146 / C2143 cascade. Fix: windows.h +               │
* * │  WIN32_LEAN_AND_MEAN + NOMINMAX BEFORE winhttp.h - exactly the           │
* * │  order the late NETRadioPlayerAtom kept (the reference                   │
* * │  verified against git). The winhttp.h -> WMFStreamSession.h order        │
* * │  was not changed; the guards mirror the WMFStreamSession.h preamble.    │
* └─────────────────────────────────────────────────────────────────────────┘
*
*
* ┌─────────────────────────────────────────────────────────────────────────┐
* * │  v2.0 CHANGES (Task 102, the URL_RADIO series - Variant A, Path 1):      │
* │                                                                         │
* * │  ALL-IN-ONE NET RADIO. The radio family is reduced to ONE atom - this.  │
* * │  NETRadioPlayerAtom (audio via the global MFPlay singleton,             │
* * │  deprecated mfplay.h) is retired; its proven ICY worker is              │
* * │  transplanted HERE as a per-instance WinHTTP thread.                    │
* │                                                                         │
* * │  NEW - the ICY METADATA WORKER (per-instance, map-keyed):               │
* * │    - reads the SAME stream URL the WMF session plays                    │
* * │    - skips audio chunks (icy-metaint), parses StreamTitle               │
* * │    - new outputs: title / artist / track (+ the "updated" impulse)      │
* * │    - its own reconnect with exponential backoff                         │
* * │    - the urlChanged flag: a hot URL change reconnects the metadata      │
* * │    - dispose: cancel + join (the RP_CancelRequest pattern)              │
* │                                                                         │
* * │  NEW - the FAULT_ISOLATION hooks (the Task 98 canon):                    │
* * │    - Play() rejected by the session      -> markAsFaulted(PLAY_FAILED)  │
* * │    - reconnect surrender (10 attempts)  -> markAsFaulted(CONN_LOST)    │
* * │    - a healthy pass (state -> PLAYING)  -> clearFault()                 │
* * │    - a deliberate stop                  -> clearFault()+clearMetadata  │
* │                                                                         │
* * │  CHANGED - a hot URL change: changing the URL while playing now         │
* * │  immediately restarts playback on the new URL (v1.5 stopped the sound   │
* * │  and waited for a manual play switch).                                  │
* └─────────────────────────────────────────────────────────────────────────┘
*
*
* ┌─────────────────────────────────────────────────────────────────────────┐
