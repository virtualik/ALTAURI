...
extern "C" void android_ws_connect(void* haxePtr, const char* url, const char* subprotocol) {
...

    {
        ...
        if (st->javaWsInstance) {
            ...
            // Correctly close the old Java instance before replacing it.
            // Java-WebSocket keeps an internal worker thread that
            // keeps trying to connect. If we simply replace the
            // global reference, the old thread keeps running and
            // its onError/onClose callbacks can still arrive in
            // our WebSocketState, causing phantom errors.
    ...
    // THE MOST IMPORTANT: checking for Java exceptions right after the call
    if (env->ExceptionCheck()) {

        LOGE(">>> [JNI] JAVA EXCEPTION during connect()!");

        env->ExceptionDescribe(); //         env->ExceptionDescribe(); // <- THIS WILL PRINT THE FULL ERROR STACK TRACE TO LOGCAT!
        env->ExceptionClear();
