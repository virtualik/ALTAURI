...
/**
 * COM PORT ATOM v2.1 (Dual-Platform: C++ WinAPI + HTML5 Web Serial API)
 *
 ...
 * └─────────────────────────────────────────────────────────────────────┘
 *
 * v2.1 Changes (HTML5 Promise fix):
 * - All Promise callbacks now use arrow functions (x) => {} instead of
 *   function(x) {} to preserve lexical `this` binding.
 * - Without this fix, `this.onPortRequested` was undefined inside
 *   .then() callbacks because regular functions create their own `this`.
 *
 * Architecture:
 * ┌─────────────────────────────────────────────────────────────────────┐
 * │                     ComPortAtom                                     │
...
// Previously: .then(function(port) { this.onPortRequested(port); })
// Now:        .then((port) => { this.onPortRequested(port); })
