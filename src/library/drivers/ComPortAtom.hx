...
static std::mutex _com_map_mutex;
   ...
   // The only place where memory for g_vm is allocated
   JavaVM* g_vm = nullptr;
   ...
   JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* reserved) {
       ...
       g_vm = vm; //        g_vm = vm; // Storing the pointer
...
extern "C" bool tryOpenAndroidUsbDevice(ComPortState* st, int baudRate, int searchVid, int searchPid, const char** outError) {
    ...
    if (values != nullptr) {
        ...
        while (env->CallBooleanMethod(iterator, hasNext)) {
            ...
            if (!deviceClass) { //             if (!deviceClass) { // PROTECTION AGAINST ZOMBIE OBJECTS
                                    ...
                                    if (port == nullptr) {
                                        ...
                                        if (portOpen == nullptr || setParams == nullptr) {
                                            ...
                                            if (env->ExceptionCheck()) {
                                                ...
                                                // This block now executes ALWAYS, even if setParameters failed
                    ...
                    if (jbuf != nullptr) {
                        ...
                        while (st->isRunning) {
                            ...
                            if (env->ExceptionCheck()) {
                                ...
                                env->ExceptionClear(); //                                 env->ExceptionClear(); // Hiding the long stack trace from Logcat
                                LOGE("Android Rx Exception -> Device Disconnected");

                                std::lock_guard<std::mutex> errLock(st->errMutex);

                                sprintf(st->errBuffer, "Device disconnected"); //                                 sprintf(st->errBuffer, "Device disconnected"); // A friendly text
                                st->hasError = true;
...

        {
...

            {
...

                {
                    ...
                    // Registry FriendlyName = a Windows-generated localized string (e.g. "USB Serial Device")
...

{
    ...
    // === For auto-opening after permissions are granted ===
    #if android
...

    {
        ...
        // === Auto-opening the port after Android permissions are granted ===
        if (_isWaitingForUsbPermission) {

            // Subtract dt, not 1.0, since update is called every frame
            ...
            if (_usbPermissionTimeout <= 0) {
                ...
                _permCheckTimer = 0.5; //                 _permCheckTimer = 0.5; // Checking permissions once per 0.5 sec, not every frame (~60 fps)
                ...
                if (hasPerm) {
                    ...
                    openDevice(); //                     openDevice(); // Trying to open it again!
                }
...

        {
            ...
            // If this is a physical cable disconnect - do not show a scary error, just close the port
            if (_pendingErrStr.indexOf("disconnected") >= 0) {
                ...
                // This is a real error (not a disconnect), show it to the user
    ...
    */
...

    {
...

        {
            ...
            // Output debug to RX only if not silent
            if (!silent) {
...

    {
...

        {
            ...
            _isWaitingForUsbPermission = false; //             _isWaitingForUsbPermission = false; // Resetting the waiting flag
            #end

            

            // === CLEARING THE ERROR, since the port opened successfully ===
            _pendingErrStr = "";
