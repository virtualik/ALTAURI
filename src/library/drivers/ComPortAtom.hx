...
// --- COMMON CALLBACKS ---
	...
	{
		if (!_isOpenFlag) return;
		_isOpenFlag = false; // FIX: Set flag immediately to prevent new operations
		_isReading = false;  // FIX: Stop read loop immediately
		...
		if (_connectionType == "serial")
		{
			...
			// FIX: Sequential close - reader -> writer -> port
			// Each step waits for the previous to complete before proceeding
			var closeSequence:Dynamic = untyped Promise.resolve();

			// Step 1: Cancel reader and release lock
			if (_reader != null)
			{
				var reader:Dynamic = _reader;
				_reader = null; // Clear reference immediately to prevent new reads
				closeSequence = closeSequence.then(function()
				{
					// FIX: Use .then(success, error) instead of ['catch'] to avoid Haxe parser bugs
			...
			// Step 2: Close writer and release lock
			if (_writer != null)
			{
				var writer:Dynamic = _writer;
				_writer = null; // Clear reference immediately
				closeSequence = closeSequence.then(function()
				{
					// FIX: Use .then(success, error) instead of ['catch']
			...
			// Step 3: Close the port itself (only after streams are released)
			if (port != null)
			{
				closeSequence = closeSequence.then(function()
				{
				...
				{
					self.onPortClosed();
				}, function(err:Dynamic) // <-- FIX: 2nd argument of .then() handles errors safely
				{
