 ...
 * │   Undo Flow:                                                            │
 * │   ─────────                                                             │
 * │   1. Remove created assembly (unregister, delete file, dispose)         │
 * │   2. Restore original atoms (definitions, instances, connections)       │
 * │   3. Restore physical connections (delayed)                             │
 * │   4. Emit events (ATOM_DELETED for assembly, ATOM_RESTORED for atoms)   │
 * │                                                                         │
 * │   v2.1 Changes:                                                         │
 * │   - FIXED: PinDef creation now uses only standard fields (name, type)   │
 * │     to avoid compilation errors if Blueprint.hx doesn't have extended   │
 * │     metadata.                                                           │
 * │                                                                         │
 * └─────────────────────────────────────────────────────────────────────────┘
 */
