...
* ║   2. NEW ATOMS_GROUPED emission in Phase 8 BEFORE the per-node            ║
* ║      ATOM_DELETED loop: { assemblyId, newAssemblyTypeId,                  ║
* ║      movedTemplateIds }. Main remaps device-window paths                   ║
* ║      [..., X] -> [..., newAssemblyTypeId, X] while resolution is          ║
* ║      still possible; whatever does not remap is pruned right after.       ║
* ║                                                                           ║
...
// PHASE 8: EMIT EVENTS
// =====================================================================
// v3.17 (Wide View WP-1a): ATOMS_GROUPED fires FIRST — before the
// per-node ATOM_DELETED loop — so path-cache owners can remap
// [..., movedId] -> [..., newAssemblyTypeId, movedId] while the new
// assembly is already linked into the parent and resolvable. Payload
// follows the EventType v1.6 Identity Contract.
                Impulsys.quickEmit(EventType.ATOMS_GROUPED, {
                        assemblyId: _assembly.id,
                        newAssemblyTypeId: newTypeId,
                        movedTemplateIds: selectedTemplateIds
                });
                for (nodeId in _selectedNodeIds)
                {
                        Impulsys.quickEmit(EventType.ATOM_DELETED, {assemblyId: _assembly.id, atomId: nodeId});
                }
                Impulsys.quickEmit(EventType.ATOM_RESTORED, {
                        assemblyId: _assembly.id,
                        atomId: newInstance.id,
                        x: centerPos.x,
                        y: centerPos.y,
                        atom: newInstance
                });
...
// Without this, the mapping would linger and point to a disposed atom.
                ...
                Impulsys.quickEmit(EventType.ATOM_DELETED, {assemblyId: _assembly.id, atomId: createdId});
                for (atomDef in _snapshot.getRemovedAtomDefs())
                {
