...
* ║  │   │  - loadMainWindowStateEarly() → Lightweight window state    │   ║  ║
* ║  │   │                               read for startup restore      │   ║  ║
...
* ║  │        }                                                            │  ║
* ║  │                                                                     │  ║
* ║  │    Describes the MAIN application window (NOT the legacy separate   │  ║
* ║  │    device window — that keeps its own "deviceWindow" section).      │  ║
* ║  │  - ADDED: saveSelfrun() optional mainWindow parameter               │  ║
* ║  │  - ADDED: loadSelfrun() returns mainWindow (null when absent)       │  ║
* ║  │  - ADDED: loadMainWindowStateEarly() — tiny standalone read used    │  ║
* ║  │    by Main.init() BEFORE the window becomes visible (no flash)      │  ║
* ║  │  - Save cadence (agreed): app exit + fullscreen toggle only         │  ║
* ║  │  - Fully backward compatible: old files without the section load    │  ║
* ║  │    with defaults; save version bumped 2.5 → 2.6                     │  ║
* ║  │                                                                     │  ║
* ║  │  v2.5 Changes:                                                     │  ║
...
class ProjectManager
{
        ...
        */
        public function init():Void
        {
                        ...
                        // Make sure the path ends with a slash so the names do not stick together.
        ...
        * v2.6: Added optional mainWindow state parameter (sibling JSON section).
        *
        * @param mainWindow Main window state collected by Main.collectMainWindowState():
        *                   {x, y, width, height, screen, maximized}. When the window is
        *                   zoomed (SW_MAXIMIZE), x/y/width/height MUST already contain
        *                   the PRE-maximize rect (Main takes it from DisplayConfig.savedWindowRect).
        */
        public function saveSelfrun(
                ...
                ?windowX:Float = 100,
                ?windowY:Float = 100,
                ?mainWindow:{x:Float, y:Float, width:Float, height:Float, screen:Int, maximized:Bool}
        ):Void
        {
                #if sys
...
// ══════════════════════════════════════════════════════════════════
// v2.6: Main application window state (sibling section)
// ══════════════════════════════════════════════════════════════════
// Describes the MAIN window only. The legacy separate device window keeps
// its own "deviceWindow" section above — the two must not be conflated.
// state: "normal" | "maximized" (human-readable JSON).
                if (mainWindow != null)
                {
                        data.mainWindow = {
                                x: mainWindow.x,
                                ...
                                state: mainWindow.maximized ? "maximized" : "normal"
                        };
                }
// v2.5: Serialize platforms field if present
                ...
                try {
                        ...
                        trace("ProjectManager: Selfrun saved (v2.6 with mainWindow state).");
                }
                catch (e:Dynamic)
                {
...
// Save nested assemblies to library
        ...
        * v2.6: Returns mainWindow state (null when the section is absent —
        *       old save files; caller falls back to defaults).
        *
...
// ══════════════════════════════════════════════════════════════════
// v2.6: Parse main window state (absent in old files → null)
// ══════════════════════════════════════════════════════════════════
                        var mainWindowState:{x:Float, y:Float, width:Float, height:Float, screen:Int, maximized:Bool} = null;
                        if (json.mainWindow != null)
                        {
                                mainWindowState = {
                                        x: _safeFloat(json.mainWindow.x, 100),
                                        y: _safeFloat(json.mainWindow.y, 100),
                                        width: _safeFloat(json.mainWindow.width, 1024),
                                        height: _safeFloat(json.mainWindow.height, 600),
                                        screen: Std.int(_safeFloat(json.mainWindow.screen, 0)),
        ...
        /**
        * v2.6: Lightweight standalone read of the mainWindow section.
        *
        * Used by Main.init() BEFORE the window becomes visible: at that point the
        * full loadSelfrun() pipeline has not run yet (project build happens later),
        * so this method reads and parses ONLY the tiny mainWindow block — cheap
        * and side-effect-free.
        *
        * @return Window state, or null when the file / section does not exist
        *         (first run, old v2.5 files, or any parse failure).
        */
        public function loadMainWindowStateEarly():Null<{x:Float, y:Float, width:Float, height:Float, screen:Int, maximized:Bool}>
        {
                ...
                {
                        return null; // Corrupted or partial file — caller uses defaults
                }
                #else
                return null;
                #end
