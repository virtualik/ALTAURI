...
class ProjectManager
{
	...
	public function deleteAssemblyFile(typeId:String):Void
	{
		...
		if (FileSystem.exists(path))
		{
			...
			catch (e:Dynamic) { trace('Error deleting $path'); }
		}
		#end
	}
...
// ============================================================================
// ProjectManager.hx - ИСПРАВЛЕННЫЙ parseBlueprintFromJson()
// ============================================================================
		...
		if (rawBp.pins != null)
		{
			for (p in (cast(rawBp.pins, Array<Dynamic>)))
			{
				pins.push(
				{
					...
					// === FIX: Используем _parseContactType для конвертации String → ContactType ===
