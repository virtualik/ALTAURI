...
// Rewrite each connection's atomId in place
        ...
        *
        * PROBLEM:
        * When a port is created via addPort() (e.g., "incoming_1"), its
        * externalName defaults to the internalName. The user later connects
        * an atom to that port, but the externalName never updates — so on
        * the parent schema the port still shows "incoming_1" instead of
        * something meaningful like "Button_out" or "Oscilloscope_in".
        *
        * GroupAtomsCommand already sets semantic externalNames at creation
        * time (e.g., "PassThrough_1_in"), but ports added later via
        * addPort() or ports whose connected atom was renamed do not get
        * this treatment.
        *
        * SOLUTION:
        * Scan blueprint.internalConnections for every SELF endpoint, find
        * the atom on the other end, and rebuild externalName as
        *   "{atomDisplayName}_{contactName}"
        * matching the GroupAtomsCommand convention.
        *
...
// by scanning SELF endpoints in connections.
...
// Build new external name (replace spaces for blueprint-safety)
                        var newExtName = StringTools.replace(info.atomName, " ", "_")
                                + "_" + info.contactName;
...
// Recreate ConductorPort with new externalName (Contact.name is immutable
// after construction, so we must rebuild the port).
