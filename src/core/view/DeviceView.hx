...
// =========================================================================
// FRAMELESS DEVICE FACE (v1.2, Task 159)
// =========================================================================
        /**
        * Does this widget want a CHROME-FREE DeviceCard when placed on the
        * DevicePanel / DeviceWindow?
        *
        * true = the card skips the title bar, the close button, the card
        * background and ALL mouse interaction: the widget renders bare
        * (PictureWidget — the "bare canvas" mode, SPEC_PICTURE §13). The
        * widget itself then owns its placement via its contacts.
        *
        * Override in subclasses that render bare on the panel; the base
        * widgets keep the standard framed card.
        *
        * @return false for the standard framed card
        */
        public function isFramelessWidget():Bool
        {
                return false;
        }
}
