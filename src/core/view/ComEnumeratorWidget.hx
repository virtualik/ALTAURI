...
/**
 * COM ENUMERATOR WIDGET
 * Widget displaying the list of available COM ports.
 *
 * Architecture: "ATOM IS DATABANK & COMPUTE CORE"
 *
 * ComEnumeratorWidget is the FACE of ComEnumeratorAtom.
 *
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │   ComEnumeratorWidget                                                   │
 * │                                                                         │
 ...
 * │   └───────────────────────────────────────────────────────────────┘     │
 * │                                                                         │
 * │   Widget READS state from atom's "ports" contact (Databank)             │
 * │   Atom updates contact via background registry scanning thread          │
 * │                                                                         │
 * └─────────────────────────────────────────────────────────────────────────┘
 *
 * Features:
 * 1. Auto-update on USB-COM adapter plug/unplug
 * 2. Port list with "Copy Name" buttons
 * 3. LED indicator for scanning activity
 * 4. Raw string display for debugging
 * 5. Detected port counter
 */
class ComEnumeratorWidget extends DeviceView
{
    ...
    // LED pulse timer for list updates
    ...
    private function clearPortItems():Void 
    {
        ...
        // Clear arrays and remove listeners
    ...
    // =========================================================================
    /**
     * Copy port name to system clipboard.
     * Note: OpenFL's System.setClipboard() relies on the underlying OS API.
     * On some platforms or sandboxed environments, this may silently fail,
     * so we wrap it in a try-catch block to prevent crashes.
     */
    private function onCopyClick(e:MouseEvent):Void 
    {
        ...
        // Copy port name to system clipboard (if available)
        try 
        {
