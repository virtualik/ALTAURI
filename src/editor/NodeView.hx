...
class NodeView extends Sprite {
	...
	*/
	private function recalcSize():Void {
		...
		if (deviceView != null) {
			...
			widgetHeight = scaledH + WIDGET_PADDING; // Только нижний padding
			trace('Widget size: ${ws.width} x ${ws.height}');
		}
		
		// --- Вычисляем высоту секции портов ---
	...
	*/
	...
	public function redraw():Void {
		...
		// Добавляем разделитель между секцией портов и виджетом
		if (deviceView != null) {
			var ws = deviceView.getWidgetSize();
			var scaledH = ws.height * PREVIEW_SCALE;
	...
	private function createPorts():Void {
		...
		// Вычисляем высоту ТОЛЬКО секции портов (без виджета)
		var portsHeight:Float = MIN_BODY_HEIGHT;
	...
	private function createPortSprite(name:String, isInput:Bool):Sprite {
		...
		label.width = 60;  // Увеличили ширину для длинных имён
		label.height = 14;
		...
		if (isInput) {
			// Для input: label справа от порта, выровнен по левому краю
			label.x = w / 2 + 3;
			label.text = name;
		} else {
			// Для output: label слева от порта, выровнен по ПРАВОМУ краю
			// Используем отрицательное смещение + ширину label
	...
	/**
	 * Создать inline редакторы для контактов без подключений.
	 * Вызывается после createPorts().
	 */
	private function createInlineEditors():Void {
		// Очистить старые редакторы
		...
		for (contact in inputs) {
			...
			editor.x = PORT_RADIUS * 2 + 5;  // Справа от порта
			editor.y = yPos;
			addChild(editor);
	...
	/**
	 * Обновить видимость редакторов при изменении соединений.
	 */
	...
	private function addWidgetToPreview():Void {
		...
		// === FIX: Сброс координат перед добавлением в NodeView ===
		deviceView.x = 0;
		deviceView.y = 0;
		// === FIX v3.3: Принудительно сбрасываем масштаб к preview ===
		...
		// Пересчитываем размеры с правильным масштабом
		updateLayout();
		
		// === FIX v3.3: Принудительно центрируем после обновления layout ===
		// Небольшая задержка чтобы виджет успел полностью инициализироваться
		if (stage != null) {
	...
	public function acceptWidget():Void {
		if (deviceView == null) {
			...
			// === FIX v3.3: Сбрасываем масштаб перед добавлением ===
			deviceView.scaleX = PREVIEW_SCALE;
			deviceView.scaleY = PREVIEW_SCALE;
			...
			// === FIX v3.3: Принудительно центрируем ===
			// Задержка чтобы убедиться что layout обновился
