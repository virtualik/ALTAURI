...
*   the entire project hierarchy, not just the current assembly.
*
* v3.6 Changes:
* - ADDED: reattachToAtom(newAtom) — swaps the atom reference after
*   EditorContext.updateInstancesOf() reconstructs an Assembly. Releases
*   the old DeviceView (deactivates Contact subscriptions on the disposed
...
* - CHANGED: assembly field is now plain (no accessor restriction) so
*   reattachToAtom can update it.
*
* v3.5.3 Changes:
* - FIXED: acceptWidget() now calls centerPreviewContainer() synchronously
*   instead of via haxe.Timer.delay(..., 50). This prevents WireRenderer
*   from running against stale port positions after Editor ↔ Device Panel
*   mode switch.
* - FIXED: getPortPosition() now returns null when the port sprite is not
*   on the stage. Previously returned (this.x, this.y) as a fallback,
*   causing wires to be drawn to wrong coordinates.
*
* v3.4 Changes:
...
* │   ↑ TextField (INPUT)                                               │
* │                                                                     │
* │  ENTER → apply (with global uniqueness check)                       │
* │  ESCAPE → cancel                                                    │
* │  Focus out → apply                                                  │
* │                                                                     │
* │  If name conflict:                                                  │
...
// =========================================================================
        ...
        *
        * Without this, the NodeView kept holding a reference to the OLD
        * (disposed) Assembly. After Assembly.dispose():
        *   - internalAtoms = null
        *   - ports = null
        *   - _inputs / _outputs = null (via super.dispose in Atom)
        *
        ...
        * use it, to also release the old DeviceView and acquire a new one.
        */
        public var atom(get, set):Atom;
        private var _atom:Atom;
        private function get_atom():Atom return _atom;
        private function set_atom(value:Atom):Atom
        {
                _atom = value;
                return value;
        }
        // v3.6: assembly is now writable so reattachToAtom can update it.
...
// =========================================================================
        ...
        /** Reference to parent Assembly for uniqueness check (legacy) */
        private var _parentAssembly:Assembly = null;

        // v3.7: Callback to check if a name is taken globally across the entire project
        public var isNameTakenGlobally:(String, ?String) -> Bool = null;
...
// =========================================================================
        ...
        /**
         * v3.8: Set to true when the node actually moves during a drag.
         * Used in onClick to suppress selection after a drag —
         * dragging a node should NOT select it.
         */
        private var _didDrag:Bool = false;

// =========================================================================
// LONG-PRESS STATE (v3.8 — Android rule #2)
// =========================================================================
        /** True while a touch sequence is active on this node. */
        private var _touchActive:Bool = false;
        /** Long-press timer (2 s hold). */
        private var _longPressTimer:haxe.Timer = null;
        /** Whether the long-press has already fired. */
        private var _longPressFired:Bool = false;
        /** Stage X where the touch began. */
        private var _touchStartX:Float = 0;
        /** Stage Y where the touch began. */
...
// =========================================================================
        ...
        */
        ...
        /**
        * v3.6: Reattaches this NodeView to a freshly reconstructed Atom instance.
        *
        * WHY THIS IS NEEDED:
        * EditorContext.updateInstancesOf() disposes the old Assembly and
        * creates a new one via AssemblyFactory.createAtom(typeId, oldRuntimeId).
        * The runtime ID is preserved, so the parent's blueprint.internalAtoms
        * reference stays valid. BUT the new Assembly is a DIFFERENT OBJECT —
        * its _inputs, _outputs, ports, internalAtoms are all new.
        *
        * Without reattach, this NodeView kept pointing at the OLD (disposed)
        * Assembly. After Assembly.dispose():
        *   - internalAtoms = null
        *   - ports = null
        ...
        *      Contact subscriptions on the OLD atom's contacts are released
        *   2. Swaps _atom to the new instance
        *   3. Updates the `assembly` reference if the new atom is an Assembly
        *   4. Re-acquires the DeviceView for the new atom (fresh subscriptions)
        *   5. Rebuilds layout + ports + inline editors against the new atom
        *
        * @param newAtom The newly constructed Atom (same runtimeId as the old
        *                one, but a different object).
        */
        public function reattachToAtom(newAtom:Atom):Void
        {
                ...
                if (deviceView != null)
                {
                        ...
                        if (_atom != null)
                        {
                                core.view.DeviceViewRegistry.getInstance().clearContainer(_atom.id);
                        }
                        deviceView = null;
                        hasWidget = false;
                }
                ...
                {
                        this.assembly = cast(newAtom, Assembly);
                }
                else
                {
                        this.assembly = null;
                ...
                // 4. Re-acquire DeviceView for the new atom (creates fresh
                //    subscriptions on the new atom's contacts)
                acquireWidget();

                // 5. Rebuild layout + ports + inline editors
                //    createPorts() will now read from the NEW atom.getInputs()
                //    and get the correct Contact array.
                updateLayout();
                createInlineEditors();
...
// =========================================================================
        ...
        public function acceptWidget():Void
        {
                if (deviceView == null)
                {
                        ...
                        deviceView.scaleY = PREVIEW_SCALE;
                        addWidgetToPreview();
                        // === v3.5.3 FIX: Synchronous centerPreviewContainer ===
                        // Previously this used haxe.Timer.delay(..., 50), which meant
                        // WireRenderer.rebuildAll() could fire BEFORE the preview
                        // container was positioned. Port sprite positions were then
                        // computed against a stale _nodeHeight/_previewContainer.y,
                        // and wires appeared collapsed to (0,0) on the parent schema
                        // after switching Editor → Device Panel → Editor.
                        //
                        // Now we center synchronously. addWidgetToPreview() already
                        // calls updateLayout() (which calls recalcSize + redraw +
                        // createPorts), so all geometry is fresh by this point and
                        // localToGlobal() returns correct stage coordinates.
                        centerPreviewContainer();
                }
...
// =========================================================================
        ...
        private function setupInteraction():Void
        {
                ...
                // v3.8: Touch handlers for Android long-press (rule #2)
        ...
        */
        ...
        {
                // === Guard 0 (v3.8): Suppress selection after drag ===
                // If the node was actually moved, this click is the terminal
                // event of a drag — do NOT select/deselect the node.
                if (_didDrag) { _didDrag = false; e.stopPropagation(); return; }
                // === Guard 0b (v3.8): Suppress after long-press ===
                if (_longPressFired) { _longPressFired = false; e.stopPropagation(); return; }
...
// === All guards passed → Start drag ===
                _didDrag = false; // v3.8: reset — will be set true in onMouseMoveDrag
...
// =========================================================================
// TOUCH HANDLERS (v3.8 -- Android long-press, rule #2)
// =========================================================================
        /**
         * TOUCH_BEGIN on this node (Android).
         *
         * Strategy: let the synthetic MOUSE_DOWN start drag tracking normally.
         * In parallel, start a 2-second long-press timer. Two outcomes:
         *
         *  A) User moves finger -> onMouseMoveDrag fires (synthetic MOUSE_MOVE),
         *     cancels long-press via _cancelNodeLongPress(). Drag proceeds normally.
         *
         *  B) User holds still 2s -> timer fires, cancels the drag listeners,
         *     emits NODE_RIGHT_CLICKED (context menu).
         *
         * We do NOT call stopPropagation() here -- the synthetic MOUSE_DOWN must
         * reach onMouseDown so that drag tracking is set up. If the user moves,
         * the drag works. If the user holds still, we cancel it from the timer.
         */
        private function onTouchBegin(e:TouchEvent):Void
        {
                ...
                _longPressTimer = haxe.Timer.delay(function():Void {
                        ...
                        _didDrag = false;
                        // Cancel drag tracking (remove stage MOUSE_MOVE/UP listeners)
                        if (view.stage != null) {
                                view.stage.removeEventListener(MouseEvent.MOUSE_MOVE, view.onMouseMoveDrag);
                                view.stage.removeEventListener(MouseEvent.MOUSE_UP, view.onMouseUpDrag);
                        }
        ...
        /**
         * TOUCH_MOVE on this node (Android).
         * Cancels long-press if finger moved beyond dead zone.
         * Also cancels long-press if the node is actually being dragged
         * (the synthetic MOUSE_MOVE handler sets _didDrag).
         */
        private function onTouchMove(e:TouchEvent):Void
        {
                if (!_touchActive) return;

                // If drag already moved the node, cancel long-press
                if (_didDrag) { _cancelNodeLongPress(); return; }
        ...
        /**
         * TOUCH_END on this node (Android).
         * Cleans up touch state and cancels any pending long-press.
         */
        ...
        /** Cancel the node long-press timer (if running). */
        private function _cancelNodeLongPress():Void
        {
...
// =========================================================================
        ...
        */
         ...
         *
         * v3.4 CHANGES (NamingService integration):
         * ─────────────────────────────────────
         *  - Global uniqueness is enforced by NamingService, NOT by the parent
         *    Assembly alone. Two atoms in DIFFERENT assemblies can no longer
         *    share the same displayName.
         *  - On conflict, the requested name is AUTO-SUFFIXED with "_N" (e.g.,
         *    "Button" → "Button_1"). No red-border rejection — the user
         *    can immediately re-edit if they want a different variant.
         *  - blueprint.name is NO LONGER touched here. Per user policy (b):
         *    blueprint.name is a unique blueprint identifier set only at
         *    creation time / "Save As..." time. Renaming an instance only
         *    affects its displayName, never its blueprint.
         */
        private function finishNameEditing():Void
        {
                if (!_isEditingName) return;
                _isEditingName = false;
                var typedName = StringTools.trim(_nameInput.text);
                // Empty name — revert
                ...
                // ═══════════════════════════════════════════════════════════════════
                // v3.4: Resolve globally-unique variant via NamingService.
                // ═══════════════════════════════════════════════════════════════════
                // NamingService returns typedName as-is if free, or "typedName_N"
                // if taken. We then atomically swap old → new in the registry.
                // ═══════════════════════════════════════════════════════════════════
                var oldName:String = atom.displayName;
                var finalName:String = core.logic.NamingService.resolveUniqueInstanceName(
                        typedName, atom.id);

                if (finalName != typedName)
                {
                        trace('NodeView: Name "$typedName" taken — applied as "$finalName"');
                }

                // Atomic rename in NamingService (unregister old, register new)
                var ok:Bool = core.logic.NamingService.renameInstance(
                        oldName, finalName, atom.id);
                if (!ok)
                {
                        // Should not happen given resolveUniqueInstanceName above,
                        // but be defensive — revert and stay in edit mode.
                        trace('NodeView: renameInstance FAILED for "$finalName"');
                        _nameInput.borderColor = 0xFF3333;
                        haxe.Timer.delay(function()
                        {
                                if (_nameInput != null) _nameInput.borderColor = 0x00AAFF;
                        }, 1000);
                        _isEditingName = true;
                        return;
                }
                ...
                // fixed at creation/save-as time. Only displayName (instance
                // label) is user-editable on the canvas.

                _titleLabel.text = finalName;
                _nameInput.visible = false;
                _titleLabel.visible = true;

                // Save project
                Impulsys.quickEmit(EventType.VALUE_COMMITTED);
                trace('NodeView: Renamed atom "$oldName" → "$finalName"');
        }
...
// =========================================================================
        ...
        /**
        * Returns the global (stage) position of a port sprite by contact name.
        *
        * v3.5.3 FIX: Now returns NULL when the port sprite is not on the stage.
        * Previously this returned (this.x, this.y) as a fallback, which caused
        * WireRenderer to draw wires to the wrong location (typically the
        * top-left corner of the canvas) when port sprites had been created
        * but not yet added to the display list — e.g., immediately after
        * createPorts() during a mode switch.
        *
        * Returning null lets WireRenderer.rebuildAll() skip the wire cleanly
        * via its existing "if (p1 == null || p2 == null) continue;" guard,
        * and the wire will be drawn on the next rebuildAll() call once the
        * port sprite is properly staged.
        *
        * @param contactName Name of the contact (matches port.name)
        * @return {x, y} in stage coordinates, or null if port not found / not staged
        */
        public function getPortPosition(contactName:String): {x:Float, y:Float}
        {
                ...
                // v3.5.3: Port sprite exists but is not yet on stage → its
                // localToGlobal would return (0,0) (the stage origin), which
                // is a valid but wrong coordinate. Bail out and let the caller
                // skip this wire until the next rebuild.
                if (port.stage == null) return null;
