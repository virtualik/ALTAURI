...
class NodeView extends Sprite {
    ...
    // ... [draw, calculateHeight, buildUI, etc. - unchanged] ...
    ...
    override private function set_visible(value:Bool):Bool {
        if (super.visible != value) {
            ...
            if (_devicePreview != null) {
                if (EditorState.isZooming()) return value;
                if (value) {
                    if (!_devicePreview.isActive) _devicePreview.activate();
                } else {
                    if (_devicePreview.isActive) _devicePreview.deactivate();
                }
            }
        }
        return value;
    ...
    private function draw():Void {
        ...
        if (selected) graphics.lineStyle(3, _theme.NODE_SELECTED_COLOR);
        else graphics.lineStyle(2, _theme.NODE_BORDER_COLOR);
        ...
        if (!Std.isOfType(atom, Assembly)) {
            try {
                ...
                if (_devicePreview != null) {
                    ...
                    switch (type) {
                        case "oscilloscope": scaleFactor = 0.125;
                        case "button": scaleFactor = 0.6;
                        case "textinput": scaleFactor = 0.6;
                        default: scaleFactor = 0.6;
                    }
                    ...
                    if (isInteractive) { _devicePreview.buttonMode = true; _devicePreview.useHandCursor = true; }
                    addChild(_devicePreview);
                    if (this.visible && !EditorState.isZooming()) _devicePreview.activate();
                }
            } catch (e:Dynamic) { trace('Error creating device preview: $e'); }
        }
    ...
    // ... [Other unchanged methods: redraw, drawPorts, calculateHeight, setEcsMode, getPortPosition, dispose] ...
    ...
    public function setEcsMode(enabled:Bool):Void { _ecsMode = enabled; }
    public function redraw():Void {
        ...
        _devicePreview = null;
        draw();
        if (parentContainer != null && index >= 0 && index < parentContainer.numChildren) parentContainer.setChildIndex(this, index);
        this.x = prevX;
        this.y = prevY;
    ...
    private function onSettingsClick(e:MouseEvent):Void {
        e.stopPropagation();
        // === REPLACE START ===
        Impulsys.emit(new Impulse(EventType.ATOM_PROPERTIES_REQUEST, {
            atom: this.atom,
            view: this
        }));
        // === REPLACE END ===
    ...
    private function onDoubleClick(e:MouseEvent):Void {
        ...
        if (_assemblyInstance.blueprint.logic != null) return;
        // === REPLACE START ===
        Impulsys.emit(new Impulse(EventType.OPEN_ASSEMBLY_REQUEST, {
            atomId: this.nodeId
        }));
        // === REPLACE END ===
    }
    ...
    private function onMouseDown(e:MouseEvent):Void {
        ...
        if (Std.isOfType(targetSprite, Sprite)) {
             ...
             if (_devicePreview != null && _devicePreview.mouseEnabled && _devicePreview.contains(s)) return;
             if (inputPorts.exists(s.name) || outputPorts.exists(s.name) || s == _settingsBtn) return;
        }
        e.stopPropagation();
        // === REPLACE START ===
        Impulsys.emit(new Impulse(EventType.NODE_CLICKED, {
            id: this.nodeId,
            view: this,
            ctrlKey: e.ctrlKey
        } ));
    ...
    private function onRightMouseDown(e:MouseEvent):Void {
        e.stopPropagation();
        // === REPLACE START ===
        Impulsys.emit(new Impulse(EventType.NODE_RIGHT_CLICKED, {
            id: this.nodeId,
            view: this,
    ...
    private function onMouseMove(e:MouseEvent):Void {
        ...
        this.x = newX;
        this.y = newY;
        if (_ecsMode) ECS.updatePosition(nodeId, newX, newY);
        // === REPLACE START ===
        Impulsys.emit(new Impulse(EventType.EDITOR_NODE_MOVED, {
            id: this.nodeId,
            view: this,
            dx: dx,
            dy: dy
    ...
    private function onMouseUp(e:MouseEvent):Void {
        ...
        if (_dragStartX != this.x || _dragStartY != this.y) {
            // === REPLACE START ===
            Impulsys.emit(new Impulse(EventType.NODE_DRAG_FINISHED, {
                id: this.nodeId,
                startX: _dragStartX,
                startY: _dragStartY,
                endX: this.x,
