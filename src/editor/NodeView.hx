...
/**
 * NodeView v2.0 (Schematic Node with Ports)
...
class NodeView extends Sprite {
    ...
    // =========================================================================
    // PORTS - КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ!
    // =========================================================================
    
    /** Input ports map: contactName -> Sprite. ЭТО ГЛАВНОЕ! Порты существуют в NodeView. */
    ...
    /** Is this NodeView currently selected? */
    // ✅ ИСПРАВЛЕНО: свойство называется selected (не isSelected)
    ...
    /** Alias for selected (for compatibility with NodeEditor) */
    ...
    /** Is the widget currently in this NodeView? False when widget moved to DeviceWindow. */
    ...
    /** Called when user double-clicks to open DeviceWindow. */
    public var onOpenDeviceWindow:NodeView -> Void;
    
    /** Called when NodeView is selected. */
    ...
    /**
     * Create a NodeView for an atom.
     * 
     * ═══════════════════════════════════════════════════════════════════════
     * ИСПРАВЛЕНИЕ v2.0:
     * ═══════════════════════════════════════════════════════════════════════
     * Конструктор теперь принимает 2 параметра: (atom, nodeId)
     * 
     * @param atom The atom to display
     * @param nodeId Unique identifier for this NodeView
     */
    public function new(atom:Atom, nodeId:String) {
        super();
        this.atom = atom;
        // ✅ ИСПРАВЛЕНО: используем переданный nodeId
        this.nodeId = nodeId;
        _theme = EditorTheme.getInstance();
        
        // Initialize ports maps
        ...
        // Create ports from atom contacts
        createPorts();
        
        // Get preview widget from Registry
        acquireWidget();
    ...
    private function buildUI():Void {
		...
		// Разрешаем двойной клик по фону узла
        _background.doubleClickEnabled = true;
        addChild(_background);
        
        // Title bar
        _titleBar = new Sprite();
        // Разрешаем двойной клик по заголовку
        ...
        // Preview container for DeviceView
    ...
    private function createPortSprite(name:String, isInput:Bool):Sprite {
        ...
        if (isInput) {
            label.x = PORT_RADIUS + 3;
        } else {
            label.x = -PORT_RADIUS - 53;
        }
        label.y = -7;
        label.text = name;
        port.addChild(label);
        ...
        // Mouse events for wire creation
    ...
    private function acquireWidget():Void {
        ...
        // Get or create widget (singleton per atom!)
        ...
        // Check if widget is currently in DeviceWindow
        ...
        // Add widget to preview container
        addWidgetToPreview();
    }
    ...
    private function addWidgetToPreview():Void {
        ...
        if (deviceView.parent != null) {
            deviceView.parent.removeChild(deviceView);
        }
        ...
        // Включаем doubleClickEnabled для виджета и всех его детей
        // Это нужно, чтобы клик по кнопке внутри превью генерировал DOUBLE_CLICK, который всплывет до NodeView
     ...
     /**
     * Рекурсивно включает doubleClickEnabled для объекта и всех его детей.
     * Это нужно, чтобы событие DOUBLE_CLICK могло всплывать от детей к родителю (NodeView).
     */
    private function enableDoubleClickRecursive(obj:openfl.display.DisplayObjectContainer):Void {
        if (obj == null) return;
        
        // Включаем для самого контейнера
        obj.doubleClickEnabled = true;
        
        // Рекурсивно проходим по детям
        for (i in 0...obj.numChildren) {
            var child = obj.getChildAt(i);
            // Если ребенок тоже контейнер (Sprite, MovieClip)
            if (Std.isOfType(child, openfl.display.DisplayObjectContainer)) {
                enableDoubleClickRecursive(cast child);
            } 
            // Если просто интерактивный объект (TextField etc)
            else if (Std.isOfType(child, openfl.display.InteractiveObject)) {
                cast(child, openfl.display.InteractiveObject).doubleClickEnabled = true;
            }
        }
    }
    ...
    private function onDoubleClick(e:MouseEvent):Void {
        ...
        // Не открываем, если клик по порту
        ...
        // Если атом — сборка, открываем её для редактирования
        if (Std.isOfType(atom, Assembly)) {
    ...
    private function onClick(e:MouseEvent):Void {
        // Don't process if clicking on port
        if (Std.isOfType(e.target, Sprite)) {
            var target:Sprite = cast e.target;
            if (inputPorts.exists(target.name) || outputPorts.exists(target.name)) {
                return;
        ...
        Impulsys.quickEmit(EventType.NODE_CLICKED, {
            view: this,
            id: nodeId,
            ctrlKey: e.ctrlKey
        });
    ...
    private function onRightClick(e:MouseEvent):Void {
        Impulsys.quickEmit(EventType.NODE_RIGHT_CLICKED, {
            view: this,
            id: nodeId,
            x: e.stageX,
            y: e.stageY
    ...
    private function onSettingsClick(e:MouseEvent):Void {
        e.stopPropagation();
        Impulsys.quickEmit(EventType.ATOM_PROPERTIES_REQUEST, {
            atom: atom,
            view: this
        });
    }
    ...
    private function onMouseDown(e:MouseEvent):Void {
        // Не перетаскиваем, если клик по порту
        if (Std.isOfType(e.target, Sprite)) {
            var target:Sprite = cast e.target;
            if (inputPorts.exists(target.name) || outputPorts.exists(target.name)) {
                return;
        ...
        // Запоминаем стартовую позицию и смещение
        _dragOffsetX = e.localX;
        _dragOffsetY = e.localY;

        // Поднимаем узел наверх
        if (parent != null) parent.addChild(this);

        // Подписываемся на движение и отпускание глобально (на stage)
        if (stage != null) {
    ...
    private function onMouseMoveDrag(e:MouseEvent):Void {
        ...
        // localToGlobal -> globalToLocal для корректной работы внутри прокрутки
        ...
        // Считаем дельту (насколько сдвинулись)
        var dx = newX - this.x;
        var dy = newY - this.y;
        ...
        if (dx != 0 || dy != 0) {
            ...
            // Обновляем ECS (если используется)
            ECS.updatePosition(nodeId, newX, newY);

            // === ГЛАВНОЕ: Уведомляем редактор, что мы движемся ===
            // Передаем дельту, чтобы редактор мог подвинуть остальные выделенные узлы
            Impulsys.quickEmit(EventType.EDITOR_NODE_MOVED, {
                id: this.nodeId,
                view: this,
                dx: dx,
                dy: dy
    ...
    private function onMouseUpDrag(e:MouseEvent):Void {
        ...
        // Сигнализируем о завершении перетаскивания
        Impulsys.quickEmit(EventType.NODE_DRAG_FINISHED, {
            view: this,
            id: nodeId
        });
    }
     ...
     * Used by WireRenderer for wire connection points.
     */
    public function getPortPosition(contactName:String):{x:Float, y:Float} {
        ...
        // Return global (stage) coordinates
    ...
    /**
     * Clean up this NodeView.
     * 
     * Note: Does NOT dispose the DeviceView!
     * DeviceView is managed by DeviceViewRegistry.
     */
    public function dispose():Void {
        ...
        // Remove widget from display list (but don't dispose!)
