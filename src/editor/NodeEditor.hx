...
/**
 * NodeEditor v3.3 (EventType Refactor)
 */
class NodeEditor extends Sprite {
    ...
    // ... [Helper methods: getViewState, setViewState, updateVisibility, restoreExistingAtoms, etc. remain unchanged] ...
    // (Я пропущу неизменные методы для краткости внутри этого блока, но в файле они должны быть)

    public function getViewState():{x:Float, y:Float, zoom:Float} return _viewport.getViewState();
    public function setViewState(state:{x:Float, y:Float, zoom:Float}):Void { _viewport.setViewState(state); updateVisibility(); }
    ...
    private function restoreExistingAtoms():Void {
        ...
        for (atomDef in _blueprint.internalAtoms) {
            ...
            if (atomInstance != null) createViewForAtom(cast atomInstance, runtimeId, atomDef.x, atomDef.y);
        }
    }
    ...
    public function deselectAll():Void {
        ...
        if (_selectedWireIds.length > 0) { _selectedWireIds = []; _wireRenderer.rebuildAll(); }
    }
    public function selectAll():Void {
        deselectAll();
        for (id in _nodes.keys()) {
            var view = _nodes.get(id);
            if (view != null) { view.selected = true; _selectedNodes.set(id, view); }
        }
    }
    public function isSelected(nodeId:String):Bool return _selectedNodes.exists(nodeId);
    public function selectNode(nodeId:String, view:NodeView):Void { view.selected = true; _selectedNodes.set(nodeId, view); }
    ...
    public function deleteAtom(id:String):Void { _actions.deleteAtoms([id]); }
    ...
    public function copySelection():Void { _actions.copySelection(getSelectedNodeIds()); }
    public function cutSelection():Void { _actions.cutSelection(getSelectedNodeIds()); }
    ...
    // Rest of helper methods that were unchanged:
