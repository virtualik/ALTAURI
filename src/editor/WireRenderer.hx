...
*
* ═══════════════════════════════════════════════════════════════════════════
* v1.7 CHANGES (Wire Long-Press — Android rule #3)
* ═══════════════════════════════════════════════════════════════════════════
*
*  Added 2-second long-press detection on wire hitSprites for Android.
*  Touch handlers (TOUCH_BEGIN/MOVE/END) are added alongside the
*  existing mouse handlers. On long-press, the wire is selected (if not
*  already) and WIRE_RIGHT_CLICKED is emitted to show the context menu.
*
* ═══════════════════════════════════════════════════════════════════════════
...
class WireRenderer {
        ...
        // === Long-press state (v1.7 — Android rule #3) ===
        private static inline var WIRE_LONG_PRESS_MS:Int = 2000;
        private static inline var WIRE_LONG_PRESS_DEAD_ZONE_SQ:Float = 100.0;
        ...
        public function rebuildAll():Void {
                ...
                // calling Graphics.clear() and lineTo() on every touch event only
                // creates unnecessary load on the CPU and GC without improving the visuals.
                // RADICAL FREEZING: Completely disables recalculation of Bezier curves and matrices during a gesture.
                // The GPU scales the _canvas, and Haxe rests.
                ...
                for (key in _wireSprites.keys()) {
                        ...
                        if (entry.hitSprite != null) {
                                ...
                                if (entry.touchBeginHandler != null) {
                                        entry.hitSprite.removeEventListener(TouchEvent.TOUCH_BEGIN, entry.touchBeginHandler);
                                }
                                if (entry.touchMoveHandler != null) {
                                        entry.hitSprite.removeEventListener(TouchEvent.TOUCH_MOVE, entry.touchMoveHandler);
                                }
                                if (entry.touchEndHandler != null) {
                                        entry.hitSprite.removeEventListener(TouchEvent.TOUCH_END, entry.touchEndHandler);
                                }
        ...
        private function createWireSprite(link:ConnectionDef):Sprite {
                ...
                // === v1.7: Touch handlers for Android long-press (rule #3) ===
                var touchBeginHandler = function(e:TouchEvent) {
                        e.stopPropagation();
                        _startWireLongPress(hitSpr, id);
                };
                var touchMoveHandler = function(e:TouchEvent) {
                        e.stopPropagation();
                        _updateWireLongPress(hitSpr, id, e.stageX, e.stageY);
                };
                var touchEndHandler = function(e:TouchEvent) {
                        e.stopPropagation();
                        _cancelWireLongPress(hitSpr);
                };
                hitSpr.addEventListener(TouchEvent.TOUCH_BEGIN, touchBeginHandler);
        ...
        // ========================================================================
        // WIRE LONG-PRESS (v1.7 — Android rule #3)
        // ========================================================================
        /**
         * Start a 2-second long-press timer on a wire hitSprite.
         * If the timer fires without the finger moving beyond the dead zone,
         * emit WIRE_RIGHT_CLICKED (context menu for the wire).
         */
        private function _startWireLongPress(hitSpr:Sprite, wireId:String):Void
        {
                _cancelWireLongPress(hitSpr);
                // Find the entry and store start position
                for (id in _wireSprites.keys()) {
                        var entry = _wireSprites.get(id);
                        if (entry != null && entry.hitSprite == hitSpr) {
                                entry.longPressStartX = hitSpr.stage.mouseX;
                                entry.longPressStartY = hitSpr.stage.mouseY;
                                var selectedIds = _getSelectedWireIds();
                                var timer = haxe.Timer.delay(function():Void {
                                        if (_isDisposed) return;
                                        // Select this wire if not already selected
                                        var currentIds = _getSelectedWireIds();
                                        if (currentIds.indexOf(wireId) == -1) {
                                                currentIds = [wireId];
                                                if (_onWireSelectionChanged != null) {
                                                        _onWireSelectionChanged(currentIds);
                                                }
                                                rebuildAll();
                                        }
                                        // Emit wire right-click for context menu
                                        Impulsys.quickEmit(EventType.WIRE_RIGHT_CLICKED, {
                                                ids: currentIds.copy(),
                                                x: entry.longPressStartX,
                                                y: entry.longPressStartY
                                        });
                                }, WIRE_LONG_PRESS_MS);
                                entry.longPressTimer = timer;
                                break;
                        }
                }
        }
        ...
        /**
         * Cancel wire long-press if finger moved beyond dead zone.
         */
        private function _updateWireLongPress(hitSpr:Sprite, wireId:String, stageX:Float, stageY:Float):Void
        {
                for (id in _wireSprites.keys()) {
                        var entry = _wireSprites.get(id);
                        if (entry != null && entry.hitSprite == hitSpr) {
                                var dx = stageX - entry.longPressStartX;
                                var dy = stageY - entry.longPressStartY;
                                if (dx * dx + dy * dy > WIRE_LONG_PRESS_DEAD_ZONE_SQ) {
                                        _cancelWireLongPress(hitSpr);
                                }
                                break;
                        }
        ...
        /**
         * Cancel a wire long-press timer by hitSprite reference.
         */
        private function _cancelWireLongPress(hitSpr:Sprite):Void
        {
                for (id in _wireSprites.keys()) {
                        var entry = _wireSprites.get(id);
                        if (entry != null && entry.hitSprite == hitSpr) {
                                if (entry.longPressTimer != null) {
                                        entry.longPressTimer.stop();
                                        entry.longPressTimer = null;
                                }
                                break;
                        }
                }
