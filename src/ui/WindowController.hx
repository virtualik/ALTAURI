...
class WindowController {
    ...
    // =========================================================================
    // NEW: Apply Transparency to specific window by Title
    // =========================================================================
    ...
    @:functionCode('
        ...
        char buf[512];
        const char* targetTitle = title.__s;
        
        debugLogNative("Searching for window to apply transparency...");
        
        // Ищем окно по заголовку
        while (hCurr != NULL) {
            GetWindowTextA(hCurr, buf, 512);
            if (strlen(buf) > 0 && strstr(buf, targetTitle) != NULL) {
                hWnd = hCurr;
                break;
            }
        ...
        if (hWnd == NULL) {
            char dbg[256];
            sprintf(dbg, "Window not found for transparency: %s", targetTitle);
            debugLogNative(dbg);
            return false;
        }

        debugLogNative("Found window. Applying transparency.");
        
        // Применяем WS_EX_LAYERED
        LONG exStyle = GetWindowLong(hWnd, GWL_EXSTYLE);
        if (!(exStyle & WS_EX_LAYERED)) {
            SetWindowLong(hWnd, GWL_EXSTYLE, exStyle | WS_EX_LAYERED);
        }

        // Прозрачность по цветовому ключу (0x000000 - черный)
        BOOL result = SetLayeredWindowAttributes(hWnd, 0x000000, 255, LWA_COLORKEY);
        ...
        char dbg[256];
        sprintf(dbg, "Transparency applied to %s. Result: %d", targetTitle, result);
        debugLogNative(dbg);
        
        return result != 0;
    ')
    private function _applyTransparencyByTitleNative(title:String):Bool {
        return false;
    }

    /**
     * Публичный метод для применения прозрачности к окну по его названию.
     * Используется для вторичных окон (например, Device Window).
     */
    public function applyTransparencyToWindow(title:String):Bool {
        return _applyTransparencyByTitleNative(title);
    }
    ...
    public function debugWindowInfo():Void {}
    public function applyTransparencyToWindow(title:String):Bool return false;
    private function debugLog(msg:String):Void {}
    #end
}