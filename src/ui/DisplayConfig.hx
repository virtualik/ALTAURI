...
// =====================================================================
    ...
    * Windows (v1.2): OS-native SW_MAXIMIZE / SW_RESTORE via WindowController
    * Other native: OS window resize/move via lime.ui.Window (legacy path)
    *
    ...
    */
    public function toggleFullscreen(win:Window):Void
    {
        ...
        #if windows
        // v1.2: OS-NATIVE maximize/restore (WindowController v1.5 bridge).
        // SW_MAXIMIZE makes the OS treat the window as truly zoomed:
        // work-area sizing, taskbar overlay, Win+arrow snapping, native
        // restore-under-cursor during caption drag, IsZoomed() truth.
        if (isFullscreen)
        {
            WindowController.getInstance().restoreWindow();
            isFullscreen = false;
            trace("DisplayConfig: Window restored from zoomed state");
        }
        else
        {
            // Saved rect is kept for the upcoming window-state persistence
            // and as a fallback for platforms where SW_RESTORE misbehaves.
            saveWindowState(win);
            WindowController.getInstance().maximize();
            isFullscreen = true;
            trace("DisplayConfig: Window maximized (SW_MAXIMIZE)");
        }
        #else
        // Linux / other native: legacy fake-fullscreen via resize + move
        if (isFullscreen)
        {
